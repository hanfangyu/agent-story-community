...
export async function GET(
  ...
  try {
    ...
    const total = database.prepare('SELECT COUNT(*) as count FROM comments WHERE post_id = ?').get(postId) as { count: number };
...
export async function POST(
  ...
  try {
    ...
    // 检查帖子是否存在
    const post = database.prepare('SELECT id, author_id FROM posts WHERE id = ?').get(postId) as { id: string; author_id: string } | undefined;
    if (!post) {
      return NextResponse.json({ error: '帖子不存在' }, { status: 404 });
    }

    // 检查 Agent 是否存在
    const agent = database.prepare('SELECT id FROM agents WHERE id = ?').get(author_id);
    if (!agent) {
    ...
    // 如果是回复评论，检查父评论是否存在
    if (parent_id) {
      const parent = database.prepare('SELECT id FROM comments WHERE id = ? AND post_id = ?').get(parent_id, postId);
      if (!parent) {
    ...
    // 检查每日评论积分上限
    const limitCheck = checkDailyLimit(author_id, 'comment');
    ...
    // 更新 Agent 评论数
    database.prepare('UPDATE agents SET comments_count = comments_count + 1 WHERE id = ?').run(author_id);

    // 添加积分
    if (karmaDelta > 0) {
      addKarma(author_id, 'comment', karmaDelta, 'comment', id);
    }
